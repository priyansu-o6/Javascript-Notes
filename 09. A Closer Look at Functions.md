...
> **Topic:** `Functions`
> Default params, closures, bind, call, apply, IIFEs, and higher-order functions.
## 📑 Table of Contents
...
> 📝 **First-Class Functions** means JavaScript treats functions as regular values. You can store them in variables, pass them as arguments, return them from other functions, and store them in objects or arrays. This is the foundation of JavaScript's functional programming capabilities.

**Higher-Order Functions** are functions that operate on other functions — either by:
- **Accepting** a function as an argument (e.g., `addEventListener`, `map`, `filter`)
- **Returning** a function as a result (e.g., `bind`, factory functions)

> 💡 **Why does this matter?** Higher-order functions let you write abstract, reusable code. Instead of writing 10 similar loops, you write one `forEach` and pass it 10 different callbacks.
...
> 🖼️ **Visual:** *A Closer Look at Functions*
...
- Functions are objects so they can also have methods.
...
// Instead of using apply + array we can use call + ...spread
...
- **Partial application** means presetting some arguments now and filling in the rest later.
...
// Without bind, 'this' inside the handler would point to the button element
lufthansa.planes = 300;
lufthansa.buyPlane = function () {
  console.log(this); // → lufthansa object (because of bind)

  this.planes++;
  console.log(this.planes); // → 301, 302, ...
};
// lufthansa.buyPlane(); // Would work directly

document
  .querySelector('.buy')
  .addEventListener('click', lufthansa.buyPlane.bind(lufthansa));
// .bind(lufthansa) ensures 'this' always refers to lufthansa, not the button
...
## Closures
> 📝 **What is a Closure?** A closure is the closed-over variable environment of the execution context in which a function was created, even after that execution context is gone. In simpler terms: **a function remembers the variables from where it was born.**

**The Closure Recipe:**
1. Create a function (`A`)
2. Inside `A`, define another function (`B`)
3. `B` uses variables from `A`'s scope
4. Return `B` from `A` (or pass it somewhere)
5. Even after `A` finishes executing, `B` still has access to `A`'s variables
...
> 🎯 **The Formal Definition:** A closure gives a function access to all the variables of its **parent functions**, even after the parent functions have returned. The variable environment is attached to the function and travels with it.
...
| Concept | Syntax / Rule |
|---------|---------------|
| Default parameter | `function(a = 1, b = 2)` |
| Call | `fn.call(thisArg, arg1, arg2)` |
| Apply | `fn.apply(thisArg, [args])` |
| Bind | `const newFn = fn.bind(thisArg)` |
| Closure | Function remembers variables from its birthplace |
| IIFE | `(function(){})()` or `(()=>{})()` |

> 🎯 **Key Insight:** Closures are the foundation of data privacy in JavaScript. Any function has access to the variable environment of the execution context in which it was created.


## 🧭 Navigation
← [📄 08. Data Structures, Modern Operators and Strings](<08. Data Structures, Modern Operators and Strings.md>) | [🏠 Home](<README.md>) | [📄 10. Working With Arrays](<10. Working With Arrays.md>) →
