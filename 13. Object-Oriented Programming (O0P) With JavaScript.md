# 🏗️ 13. Object-Oriented Programming (O0P) With JavaScript

> **Topic:** `OOP`
> Prototypes, constructor functions, ES6 classes, inheritance, and encapsulation.
## 📑 Table of Contents

- [What is Object Oriented Programming?](#what-is-object-oriented-programming)
- [00P in JavaScript](#00p-in-javascript)
- [00P in JavaScript](#00p-in-javascript)
...
> 📝 **OOP in Plain English:** Object-Oriented Programming is a paradigm where you organize code into **objects** that contain both data (properties) and behavior (methods). Instead of writing a long list of instructions, you model your program as interacting objects — just like the real world.

**The Four Pillars of OOP:**

| Pillar | Real-World Analogy | In Code |
|--------|-------------------|---------|
| **Abstraction** | Car dashboard hides engine complexity | Hide internal details, expose simple interface |
| **Encapsulation** | Bank vault keeps money secure | Keep data private, control access via methods |
| **Inheritance** | Child inherits parent's traits | Child class reuses parent class code |
| **Polymorphism** | "Open" works on doors, drawers, apps | Same method name behaves differently per class |

> 💡 **Why OOP?** It helps you organize large codebases, reduce duplication, and model real-world relationships in your software.

> 🖼️ **Visual:** *Object Oriented Programming (O0P) With JavaScript*
...
3. **Link** the object's `__proto__` to the constructor's `prototype`
4. **Return** the object automatically (unless you explicitly return another object)

**Why Constructor Functions?** Before ES6 classes (2015), constructor functions were the standard way to create reusable object blueprints in JavaScript. They simulate classes using functions and prototypes.

> ⚠️ **Important:** Always use `new` with constructor functions. Calling without `new` sets `this` to the global object (or `undefined` in strict mode), polluting the global scope or failing silently.

- A **constructor function** is identical to a regular function except for how it's called. Prefixing with `new` triggers four special steps that create and initialize an object.
...
## Prototypes
> 📝 **The Prototype Chain:** Every JavaScript object has an internal link (`__proto__`) to another object called its **prototype**. When you access a property that doesn't exist on an object, JavaScript walks up the prototype chain looking for it.

**Prototype vs. `__proto__`:**
- `Constructor.prototype` is the **template** for all instances
- `instance.__proto__` is the **link** to that template
- `Object.getPrototypeOf(instance)` is the modern way to read the link

> 🎯 **Key Insight:** `Person.prototype` is NOT the prototype of `Person`. It IS the prototype of all objects created by `new Person()`. This naming confusion trips up almost every beginner.


```js
// Prototypes
console.log(Person.prototype); // → {constructor: ƒ, calcAge: ƒ}

// Add a method to the prototype — all instances inherit it
Person.prototype.calcAge = function () {
  console.log(2047 - this.birthYear);
};

ahmed.calcAge();  // → 50 (2047 - 1997)
salaria.calcAge(); // → 49 (2047 - 1998)

console.log(ahmed.__proto__); // → Person.prototype

// Where does __proto__ come from? Step 3 of the 'new' process:
// 1. New {} is created
// 2. function is called, this = {}
// 3. {} is linked to prototype (__proto__ = Constructor.prototype)
...
console.log(jonas.__proto__.__proto__.__proto__); // → null (chain ends)
...
> 📝 **Data Privacy in JavaScript:** True encapsulation (private data) has historically been difficult in JavaScript. The language has evolved through three privacy conventions:

| Approach | Syntax | Truly Private? |
|----------|--------|----------------|
| Convention | `_property` (underscore) | ❌ No — just a hint |
| Closures | Variables in constructor scope | ✅ Yes — but per-instance |
| ES2022 Private Fields | `#property` | ✅ Yes — language-level |

> 💡 **Best Practice:** Use ES2022 private fields (`#balance`) in modern projects. For older codebases or when you need shared private methods, use closures or WeakMaps.
...
| Concept | Rule / Syntax |
|---------|---------------|
| Constructor function | `const Person = function(name){ this.name = name; }` |
| Prototype method | `Person.prototype.greet = function(){}` |
| ES6 Class | `class Person { constructor(name){} greet(){} }` |
| Inheritance | `class Student extends Person` |
| `super()` | Call parent constructor |
| Private field | `#balance = 0` |
| Static method | `static helper(){}` — called on class, not instance |

> 🎯 **Key Insight:** JavaScript uses prototypal inheritance under the hood. `class` is syntactic sugar over constructor functions and `prototype`.


## 🧭 Navigation
← [📄 12. Advanced DOM and Events](<12. Advanced DOM and Events.md>) | [🏠 Home](<README.md>) | [📄 14. Mapty App - OOP, Geolocation, External Libraries, and More!](<14. Mapty App - OOP, Geolocation, External Libraries, and More!.md>) →
