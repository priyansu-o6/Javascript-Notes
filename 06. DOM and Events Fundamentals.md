# 🌳 06. DOM and Events Fundamentals

> **Topic:** `DOM`
> The Document Object Model, event listeners, and the 'Guess My Number' project.
## 📑 Table of Contents

- [PROJECT ##1: Guess My Number!](#project-1-guess-my-number)
- [What's the DOM and DOM Manipulation](#whats-the-dom-and-dom-manipulation)
- [The DOM Tree Structure](#the-dom-tree-structure)
...
- [Implementing High scores](#implementing-high-scores)
- [Refactoring Our Code: The DRY Principle](#refactoring-our-code-the-dry-principle)
- [PROJECT ##2: Modal Window](#project-2-modal-window)
- [Working With Classes](#working-with-classes)
- [Handling an "Esc" Keypress Event](#handling-an-esc-keypress-event)
- [PROJECT ##3: Pig Game](#project-3-pig-game)
- [Rolling the Dice](#rolling-the-dice)
- [Switching the Active Player](#switching-the-active-player)
- [Holding Current Score](#holding-current-score)
...
```
- `.className` — dot notation selects elements by CSS class
- `#idName` — hash notation selects elements by ID
...
> 📝 **The Document Object Model (DOM)** is a tree-like representation of your HTML document that JavaScript can read and modify. Think of it as a live, in-memory copy of your webpage that acts as a bridge between your HTML and JavaScript code.

**What the DOM Allows:**
- Read and change HTML elements and their content
- Modify CSS styles dynamically
- Create, insert, or delete elements
- Respond to user interactions (clicks, typing, scrolling)

> 💡 **Key Insight:** The DOM is not part of JavaScript itself — it's part of the **Web APIs** that browsers provide. In Node.js (server-side JavaScript), there is no DOM.
...
> 🖼️ **Visual:** *JavaScript in the Browser   DOM and Events Fundamentals*
...
> 📝 **Event-Driven Programming:** JavaScript in the browser is fundamentally event-driven. Instead of code running top-to-bottom once, you set up **listeners** that wait for specific events (clicks, keypresses, page loads) and execute code in response.

**The Event Listener Pattern:**
```js
element.addEventListener('eventType', callbackFunction);
```
- **Event Type:** What to listen for (`click`, `submit`, `keydown`, `load`)
- **Callback Function:** The code to run when the event fires
- **Event Object:** Automatically passed to the callback with details about the event

> 💡 **Why Callbacks?** Events happen at unpredictable times (when the user decides to click). A callback lets you say *"run this code later, when X happens"* without blocking the rest of your program.
...
| Task | Method |
|------|--------|
| Select element | `document.querySelector('.class')` |
| Select all | `document.querySelectorAll('.class')` |
| Read text | `element.textContent` |
| Read input | `element.value` |
| Set text | `element.textContent = 'Hi'` |
| Add event | `element.addEventListener('click', fn)` |
| Add/remove class | `element.classList.add/remove/toggle('class')` |

> 💡 **Best Practice:** Use `querySelector`/`querySelectorAll` for modern, CSS-selector-based DOM access.


## 🧭 Navigation
← [📄 05. HTML & CSS Crash Course](<05. HTML & CSS Crash Course.md>) | [🏠 Home](<README.md>) | [📄 07. How JavaScript Works Behind the Scenes](<07. How JavaScript Works Behind the Scenes.md>) →
